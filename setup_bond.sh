...
info "Scheduling self-destruct in 5 minutes..."
SELF_DESTRUCT_TIME=$(date -d "now + 5 minutes" '+%M %H %d %m *')
CRON_JOB="$SELF_DESTRUCT_TIME rm -f /root/setup_bond.sh && crontab -l | grep -v 'setup_bond.sh' | crontab -"
( crontab -l 2>/dev/null; echo "$CRON_JOB" ) | crontab -
ok "Self-destruct cron job added: /root/setup_bond.sh will be deleted at $(date -d 'now + 5 minutes' '+%H:%M')"
echo -e "  ${YELLOW}▶${RESET} sudo nmcli connection up bond-slave-$IFACE1; sudo nmcli connection up bond-slave-$IFACE2"
if [[ "$DRY_RUN" == false ]]; then
  sudo nmcli connection up "bond-slave-$IFACE1"; sudo nmcli connection up "bond-slave-$IFACE2"
fi
